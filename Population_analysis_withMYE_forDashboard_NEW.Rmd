---
title: "Domestic and International migration"
...
MYE2022_DMdata <- input_MYE2022data %>%
  mutate(Dom_A_in_K = Internal.Migration.Inflow/1000) %>% 
  mutate(Dom_B_out_K = Internal.Migration.Outflow/1000) %>%
  mutate(Dom_C_net_K = Internal.Migration.Net/1000) %>%
  mutate(Name = fct_reorder(Name, Dom_C_net_K)) %>%
  data.frame()

MYE2022_DMdata_Net <- MYE2022_DMdata %>% 
  select(Dom_C_net_K, Name) %>%
  data.frame()


MYE2022_DMdata_netzero <- MYE2022_DMdata %>% 
  mutate(Dom_C_net_K = replace(Dom_C_net_K, Dom_C_net_K != 0, 0)) %>%
  pivot_longer(
    cols = c(Dom_A_in_K, Dom_B_out_K, Dom_C_net_K),
    names_to = "Flow",
    names_prefix = "Flow",
    values_to = "Value",
    values_drop_na = TRUE) %>%
...
```{r fig_dom_mig_line_abs2, fig.height = 9, fig.width = 9,  echo=FALSE, warning=FALSE, message=FALSE}

dom_mig_bar <- ggplot() +
  geom_bar(data=MYE2022_DMdata_netzero, aes(x=Value, y=Name, fill=Flow), stat="identity", position=position_dodge()) +
  geom_point(data = MYE2022_DMdata_Net, aes(x= Dom_C_net_K, y = Name), Colour = "black", size = 2) +
 theme_gla()  +
  scale_fill_manual(name = "Migration", values=c( "#943fa6", "#63c5b5", "black"), labels = c("Domestic in-flow", "Domestic out-flow", "Net Domestic Migration"))
  
dom_mig_bar


ggsave (filename = "Q:/Teams/D&PA/Demography/Londons_Young_Children/Dom_mig_London_boroughs.svg",
         plot = dom_mig_bar,
         device = "svg",
         dpi=600,
         width = 9,
...
ggsave (filename = "Q:/Teams/D&PA/Demography/Londons_Young_Children/23_C_Int_mig_London_boroughs.svg",
         plot = int_mig_bar,
         device = "svg",
         dpi=600,
         width = 9,
